...
Declare @initial_lat float = 0; 
Declare @initial_long float = 90;
...
select count(Id)  from Database1.dbo.GeoCity;

select City, Distance 
From (
	select destination.City, -- берем название города/местности
	6373 * 2 * ASIN(SQRT(POWER(SIN((@initial_lat - destination.lat) * pi()/180 / 2),2) + COS(@initial_lat * pi()/180 ) * COS(destination.lat * pi()/180) * POWER(SIN((@initial_long - destination.long) *pi()/180 / 2), 2) ))  -- считаем формуле гаверсисинуса(Haversine) - расстояние между двумя точками на сфере
	as Distance 
	from 
